...
function SkillCard({ skill, index, isInView }: { skill: typeof skills[0]; index: number; isInView: boolean }) {
  ...
  return (
    ...
    >
      <div className="glass-card rounded-xl p-4 flex flex-col items-center gap-3 cursor-pointer transition-all duration-300 group-hover:shadow-[0_0_30px_hsl(var(--primary)/0.3)] group-hover:border-primary/50">
        <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${categoryColors[skill.category]} flex items-center justify-center transition-transform duration-300 group-hover:scale-110`}>
          <Icon className="w-6 h-6 text-primary-foreground" />
        </div>
        <span className="font-medium text-sm text-foreground text-center group-hover:text-primary transition-colors">
          {skill.name}
        </span>
      </div>
    </motion.div>
        ...
        >
          {categories.map((category, index) => (
            <motion.span
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium bg-gradient-to-r ${categoryColors[category]} text-primary-foreground`}
            >
              {category}
            </motion.span>
          ))}
